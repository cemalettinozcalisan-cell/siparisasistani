-- ============================================================
-- SipariÅŸAsistanÄ± - Supabase Database Schema (MVP)
-- ============================================================
...
create type order_status as enum ('new', 'approved', 'preparing', 'shipped', 'completed', 'cancelled');
...
create trigger trg_settings_updated_at before update on settings
  for each row execute function update_updated_at_column();
-- ============================================================
-- SipariÅŸAsistanÄ± - Seed Data (Demo)
-- ============================================================

-- Ã–rnek Tenant: Ahmet Ä°pek SucuklarÄ±
insert into tenants (id, company_name, domain, phone, email, iban, address, city, tax_number)
values (
  'a0000000-0000-0000-0000-000000000001',
  'Ahmet Ä°pek SucuklarÄ±',
  'ahmetipek',
  '05321234567',
  'info@ahmetipek.com',
  'TR12 0001 2345 6789 0001 2345 67',
  'KÃ¼Ã§Ã¼k Sanayi Sitesi No:42',
  'Afyonkarahisar',
  '1234567890'
);

-- Ã–rnek KullanÄ±cÄ± (Patron)
...
values (
  ...
  'ahmet@ahmetipek.com',
  '05321234567',
  '$2a$10$placeholder_hash', -- gerÃ§ek hash ile deÄŸiÅŸtirilmeli
  'owner'
);

-- Ã–rnek ÃœrÃ¼nler (Ahmet Ä°pek SucuklarÄ± - gerÃ§ek et Ã¼rÃ¼nleri)
insert into products (id, tenant_id, product_name, category, price, unit) values
  ...
  ('c0000000-0000-0000-0000-000000000002', 'a0000000-0000-0000-0000-000000000001', 'AcÄ±lÄ± Parmak Sucuk', 'Sucuk', 920, 'KG'),
  ...
  ('c0000000-0000-0000-0000-000000000005', 'a0000000-0000-0000-0000-000000000001', 'PastÄ±rma', 'PastÄ±rma', 1200, 'KG'),
...
-- Ã–rnek SipariÅŸ (3 KG Dana Parmak Sucuk + 1 KG PastÄ±rma)
...
insert into order_items (order_id, product_id, product_name, quantity, unit, unit_price, total) values
  ...
  ('e0000000-0000-0000-0000-000000000001', 'c0000000-0000-0000-0000-000000000005', 'PastÄ±rma', 1, 'KG', 1200, 1200);
-- ============================================================
-- SipariÅŸAsistanÄ± - Event Bus + Yeni ModÃ¼ller (ADIM 8-13)
-- ============================================================
...
-- mevcut message alanÄ±nÄ± body'ye kopyala
...
-- ORDER tablosundaki kargo alanlarÄ±nÄ± kaldÄ±r (artÄ±k shipments'te)
-- Bu migration'Ä± production'da dikkatli yapmak lazÄ±m, ÅŸimdilik yorum
...
create policy tenant_isolation_activity on activity_logs
  for all using (tenant_id = auth.uid()::uuid);
-- ============================================================
-- SipariÅŸAsistanÄ± - Son Ä°yileÅŸtirmeler (v1.0)
-- ============================================================

-- 1) TENANT SETTINGS (eski settings tablosunun yerine geÃ§er)
...
-- 4) KNOWLEDGE ARTICLES (ÅŸimdilik sadece ÅŸema, ileride kullanÄ±lacak)
...
create trigger trg_knowledge_updated_at before update on knowledge_articles
  for each row execute function update_updated_at_column();
-- ============================================================
-- SipariÅŸAsistanÄ± - Soft Delete (v1.0)
-- ============================================================

-- TÃ¼m tablolara deleted_at alanÄ± ekle
-- HiÃ§bir tabloda gerÃ§ek DELETE yok, hepsi soft delete
...
-- ORDER tablosuna mÃ¼ÅŸteri notu ekle (Conversation Replay iÃ§in)
...
-- ============================================================
-- SipariÅŸAsistanÄ± - AI Brain + Maintenance Mode
-- ============================================================

-- Maintenance Mode iÃ§in tenant_settings'e alan ekle
alter table tenant_settings add column if not exists maintenance_mode boolean not null default false;
alter table tenant_settings add column if not exists maintenance_message text default 'Åu an sipariÅŸ hizmetimiz geÃ§ici olarak kullanÄ±lamamaktadÄ±r.';

-- Conversation session'larÄ± iÃ§in (webhook giriÅŸ/Ã§Ä±kÄ±ÅŸ arasÄ± kayÄ±t)
...
create policy tenant_isolation_conv_sessions on conversation_sessions
  for all using (tenant_id = auth.uid()::uuid);
-- ============================================================
-- SipariÅŸAsistanÄ± - Session + Provider Ä°yileÅŸtirmeleri
-- ============================================================

-- 1) conversation_sessions geliÅŸtirme
...
-- 3) trigger: session oluÅŸurken label otomatik Ã¼retilsin
...
-- ============================================================
-- SipariÅŸAsistanÄ± - Golden Voice Rules
-- ============================================================
...
-- ============================================================
-- SipariÅŸAsistanÄ± - Production Ready (v1.0)
-- ============================================================
...
alter table tenant_settings add column if not exists after_hours_message text
  default 'SipariÅŸinizi memnuniyetle not alÄ±yorum. Ä°ÅŸletmemiz mesai saatleri iÃ§inde sipariÅŸinizi onaylayacaktÄ±r.';
...
-- 4) AI LEARNING QUEUE (insan dÃ¼zeltmeleri)
...
-- 5) ORDER LOCK (Ã§ift Ã§alÄ±ÅŸan engeli)
...
create policy tenant_isolation_order_locks on order_locks
  for all using (order_id in (select id from orders where tenant_id = auth.uid()::uuid));
-- ============================================================
-- SipariÅŸAsistanÄ± - Voice Engine (TTS)
-- ============================================================
...
create policy tenant_isolation_voice_cache on voice_cache
  for all using (tenant_id = auth.uid()::uuid);
-- ============================================================
-- SipariÅŸAsistanÄ± - Voice Engine Pro v2
-- ============================================================
...
create policy tenant_isolation_voice_logs on voice_logs
  for all using (tenant_id = auth.uid()::uuid);
-- ============================================================
-- SipariÅŸAsistanÄ± - Telephony + Call Recordings
-- ============================================================

-- Call sessions iÃ§in status alanlarÄ± (mevcut conversation_sessions geliÅŸtirme)
...
create policy tenant_isolation_call_recordings on call_recordings
  for all using (tenant_id = auth.uid()::uuid);
-- ============================================================
-- SipariÅŸAsistanÄ± - CRM Timeline + Order Status
-- ============================================================
...
-- activity_logs'a channel alanÄ± ekle
...
-- ============================================================
-- SipariÅŸAsistanÄ± - CRM Timeline + Order Status (DÃœZELTÄ°LMÄ°Å)
-- ============================================================

-- Mevcut enum'a yeni deÄŸerler ekle (sadece ekleme yapabiliriz)
...
-- activity_logs'a channel ve event_icon alanÄ± ekle
alter table activity_logs add column if not exists channel text
  check (channel in ('VOICE','WHATSAPP','WEB','PANEL','SYSTEM'));
alter table activity_logs add column if not exists event_icon text;
...
alter table orders add column if not exists customer_note text;
-- ============================================================
-- SipariÅŸAsistanÄ± - Product Engine Pro (Esnaf Geri Bildirimi)
-- ============================================================
...
create policy tenant_isolation_campaigns on campaigns
  for all using (tenant_id = auth.uid()::uuid);
-- ============================================================
-- SipariÅŸAsistanÄ± - Brand Voice + Son Ä°yileÅŸtirmeler
-- ============================================================
...
-- 1. Cari Hesap iÃ§in mÃ¼ÅŸteri bakiyesi
...
-- 3. SatÄ±ÅŸ tiplerine Tepsi/Palet ekle (enum degil text array)
-- products.sale_types zaten jsonb/text[] oldugu icin ekstra islem gerekmez

-- 4. MÃ¼ÅŸteri bazlÄ± Ã¶zel fiyat listesi
...
-- ============================================================
-- SipariÅŸAsistanÄ± - Mevcut Tenant'a Veri Ekle
-- (tenant zaten var, Ã¼rÃ¼n/kampanya ekle)
-- ============================================================
...
begin
  ...
  insert into products (tenant_id, product_name, category, price, unit, sale_types, variable_weight, avg_weight_gr, min_weight_gr, max_weight_gr, ai_rules) values
    (v_tenant_id, 'Dana Parmak Sucuk', 'Sucuk', 890, 'KG', '["KG","SAP"]', true, 600, 500, 700, 'Sap aÄŸÄ±rlÄ±ÄŸÄ± Ã¼retime gÃ¶re 500-700gr arasÄ± deÄŸiÅŸir. Net fiyat tartÄ±mdan sonra hesaplanÄ±r.'),
    (v_tenant_id, 'AcÄ±lÄ± Parmak Sucuk', 'Sucuk', 920, 'KG', '["KG","SAP"]', true, 600, 500, 700, 'Sap aÄŸÄ±rlÄ±ÄŸÄ± deÄŸiÅŸkendir.'),
    ...
    (v_tenant_id, 'PastÄ±rma', 'PastÄ±rma', 1200, 'KG', '["KG"]', false, null, null, null, null),
  ...
  values (v_tenant_id, 'Yaz KampanyasÄ±', '2 kilo ve Ã¼zeri sucuk alÄ±mlarÄ±nda yarÄ±m kilo kavurma %20 indirimli',
    '2 KG ve Ã¼zeri sucuk alÄ±mÄ±nda', '500 gr Kavurma %20 indirimli', 2, 'Sucuk',
    '2026-01-01', '2026-12-31', true)
  on conflict do nothing;
end $$;
-- ============================================================
-- SipariÅŸAsistanÄ± - GerÃ§ek Tenant Seed (Test Verisi)
-- ============================================================

-- Tenant: Ahmet Ä°pek SucuklarÄ±
insert into tenants (id, company_name, domain, phone, email, iban, address, city, tax_number)
values ('11111111-1111-1111-1111-111111111111', 'Ahmet Ä°pek SucuklarÄ±', 'ahmetipek',
  '05321234567', 'info@ahmetipek.com',
  'TR12 0001 2345 6789 0001 2345 67',
  'KÃ¼Ã§Ã¼k Sanayi Sitesi No:42', 'Afyonkarahisar', '1234567890')
on conflict (id) do nothing;

-- KullanÄ±cÄ±
insert into users (id, tenant_id, name, email, phone, password, role)
values ('22222222-2222-2222-2222-222222222222', '11111111-1111-1111-1111-111111111111',
  'Ahmet Ä°pek', 'ahmet@ahmetipek.com', '05321234567', '$2a$10$placeholder', 'owner')
on conflict (id) do nothing;

-- Tenant Settings (Brand Voice + YÃ¶resel)
insert into tenant_settings (tenant_id, voice_gender, brand_voice, greeting_style, ai_style,
  iban_enabled, human_transfer_enabled, callback_enabled, record_calls)
...
-- ÃœrÃ¼nler (GerÃ§ek esnaf Ã¼rÃ¼nleri - Ã§oklu satÄ±ÅŸ tipi ile)
insert into products (tenant_id, product_name, category, price, unit, sale_types, variable_weight, avg_weight_gr, min_weight_gr, max_weight_gr, ai_rules) values
  ('11111111-1111-1111-1111-111111111111', 'Dana Parmak Sucuk', 'Sucuk', 890, 'KG', '["KG","SAP"]', true, 600, 500, 700, 'Sap aÄŸÄ±rlÄ±ÄŸÄ± Ã¼retime gÃ¶re 500-700gr arasÄ± deÄŸiÅŸir. Net fiyat tartÄ±mdan sonra hesaplanÄ±r.'),
  ('11111111-1111-1111-1111-111111111111', 'AcÄ±lÄ± Parmak Sucuk', 'Sucuk', 920, 'KG', '["KG","SAP"]', true, 600, 500, 700, 'Sap aÄŸÄ±rlÄ±ÄŸÄ± deÄŸiÅŸkendir.'),
  ...
  ('11111111-1111-1111-1111-111111111111', 'PastÄ±rma', 'PastÄ±rma', 1200, 'KG', '["KG"]', false, null, null, null, null),
...
values ('33333333-3333-3333-3333-333333333333', '11111111-1111-1111-1111-111111111111',
  'Mehmet YÄ±lmaz', '05339876543', 'mehmet@example.com',
  'Ã‡ankaya Mah. No:10', 'Ankara')
on conflict (id) do nothing;
...
values ('11111111-1111-1111-1111-111111111111',
  'Yaz KampanyasÄ±',
  '2 kilo ve Ã¼zeri sucuk alÄ±mlarÄ±nda yarÄ±m kilo kavurma %20 indirimli',
  '2 KG ve Ã¼zeri sucuk alÄ±mÄ±nda',
...
-- ============================================================
-- SipariÅŸAsistanÄ± - Core Policy + Kargo AyarlarÄ±
-- ============================================================
...
-- ============================================================
-- SipariÅŸAsistanÄ± - Alias Engine + Clarification
-- ============================================================

-- Product aliases (Ã¼rÃ¼n eÅŸ anlamlÄ±larÄ±)
...
-- ============================================================
-- SipariÅŸAsistanÄ± - Demo Tenant Seed (KapsamlÄ±)
-- ============================================================
...
declare
  ...
  v_product_ids uuid[];
  v_product_names text[] := array['Dana Parmak Sucuk','AcÄ±lÄ± Parmak Sucuk','Kangal Sucuk','PastÄ±rma','Kavurma','Tulum Peyniri','Afyon Kaymak'];
  v_product_prices numeric[] := array[890,920,750,1200,650,380,450];
  v_customer_names text[] := array['Mehmet YÄ±lmaz','AyÅŸe Demir','Ali Kaya','Fatma Åahin','Mustafa Ã–ztÃ¼rk','Zeynep Arslan','Ä°brahim YÄ±ldÄ±z','Hatice Ã‡elik','Ahmet Kurt','Elif KoÃ§'];
...
begin
  -- Tenant
  insert into tenants (id, company_name, domain, phone, email, iban, address, city, tax_number)
  values (v_tenant_id, 'Demo Ä°ÅŸletme', 'demo', '05320000000', 'demo@demo.com',
    'TR12 0001 2345 6789 0001 2345 67', 'AtatÃ¼rk Cad. No:42', 'Afyonkarahisar', '1234567890')
  on conflict (id) do nothing;
  ...
  for i in 1..array_length(v_product_names, 1) loop
    ...
    values (v_tenant_id, v_product_names[i], 'Kasap ÃœrÃ¼nleri', v_product_prices[i], 'KG',
      (case when v_product_names[i] like '%Sucuk%' then '["KG","SAP"]' else '["KG"]' end)::jsonb)
    on conflict do nothing;
  end loop;

  -- MÃ¼ÅŸteriler (20 adet)
  for i in 1..20 loop
    ...
    values (v_customer_id, v_tenant_id,
      ...
      case when i % 3 = 0 then 'Ä°stanbul' when i % 3 = 1 then 'Ankara' else 'Afyonkarahisar' end)
    on conflict (id) do nothing;

    -- SipariÅŸler (50 adet = 20*50 = 1000 toplam)
    for j in 1..50 loop
      ...
      values (v_tenant_id, 'order', v_order_id, 'ORDER_CREATED',
        ...
        ' Ã¼zerinden ' || v_customer_names[1 + (i % 10)] || ' adÄ±na sipariÅŸ oluÅŸturdu.',
        case when v_channel = 'whatsapp'::order_channel then 'WHATSAPP' else 'VOICE' end, 'ğŸ›’', 'AI', v_date)
      on conflict do nothing;
    end loop;
  end loop;
  ...
  -- Kampanya
  insert into campaigns (tenant_id, title, description, condition, offer, min_quantity, target_product, start_date, end_date, active)
  values (v_tenant_id, 'Yaz KampanyasÄ±', '3 KG ve Ã¼zeri sucuk alÄ±mlarÄ±nda 500 gr kavurma hediye',
    '3 KG ve Ã¼zeri sucuk alÄ±mÄ±nda', '500 gr Kavurma Hediye', 3, 'Sucuk',
    '2026-01-01', '2026-12-31', true)
  on conflict do nothing;
  ...
  for i in 1..5 loop
    ...
    values (v_tenant_id, 'complaint', 'COMPLAINT_OPEN',
      'âš ï¸ AI, ' || v_customer_names[1 + (i % 10)] || ' iÃ§in ÅŸikayet kaydÄ± oluÅŸturdu.',
      'VOICE', 'âš ï¸', 'AI',
...
update customers set name = 'Mehmet YÄ±lmaz' where phone = '05321234567' and tenant_id = '00000000-0000-0000-0000-000000000001';
update customers set name = 'AyÅŸe Demir' where phone = '05339876543' and tenant_id = '00000000-0000-0000-0000-000000000001';
update customers set name = 'Ali Kaya' where phone = '05411223344' and tenant_id = '00000000-0000-0000-0000-000000000001';
update customers set name = 'Fatma Åahin' where phone = '05449876543' and tenant_id = '00000000-0000-0000-0000-000000000001';
update customers set name = 'Mustafa Ã–ztÃ¼rk' where phone = '05551234567' and tenant_id = '00000000-0000-0000-0000-000000000001';
update customers set name = 'Zeynep Arslan' where phone = '05328765432' and tenant_id = '00000000-0000-0000-0000-000000000001';
update customers set name = 'Ä°brahim YÄ±ldÄ±z' where phone = '05438765432' and tenant_id = '00000000-0000-0000-0000-000000000001';
update customers set name = 'Hatice Ã‡elik' where phone = '05559876543' and tenant_id = '00000000-0000-0000-0000-000000000001';
update customers set name = 'Ahmet Kurt' where phone = '05321239876' and tenant_id = '00000000-0000-0000-0000-000000000001';
update customers set name = 'Elif KoÃ§' where phone = '05411239876' and tenant_id = '00000000-0000-0000-0000-000000000001';
-- Business Settings v2 â€” Day-based hours, payments, delivery, cargo rules
...
-- Fix user_role enum â€” add manager and staff
alter type user_role add value 'manager';
alter type user_role add value 'staff';
-- Marketing v1 â€” birthday column + notification type
...
-- Invoice module â€” company name for customers
alter table customers add column if not exists company_name text;
-- ============================================================
-- SipariÅŸAsistanÄ± - SaaS Management (Abonelik & FaturalandÄ±rma)
-- ============================================================
