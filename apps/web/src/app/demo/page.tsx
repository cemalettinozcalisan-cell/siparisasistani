  ...
  return (
    <div className="p-6 space-y-4">
      <div className="flex items-center gap-3">
        <h1 className="text-2xl font-bold text-gray-900">Demo Modu</h1>
        <span className="px-2 py-0.5 bg-yellow-100 text-yellow-800 text-xs font-semibold rounded">AKTIF</span>
      </div>
      <p className="text-sm text-gray-500">API anahtari olmadan sistemi test et.</p>
        ...
        <div className="grid grid-cols-2 gap-3">
          {(scenarios as { id: string; name: string; description: string }[]).map((s) => (
            <button key={s.id} onClick={() => startScenario(s.id)} disabled={loading}
              className={`p-4 rounded-xl border-2 text-left transition-all ${selectedId === s.id ? 'border-blue-500 bg-blue-50' : 'border-gray-200 bg-white hover:border-blue-300'}`}>
          ...
          <div className="bg-white rounded-xl border border-gray-200 p-4 space-y-3 max-h-96 overflow-y-auto">
            {conversation.map((msg, i) => (
              <div key={i} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                <div className={`max-w-lg px-4 py-2 rounded-xl text-sm ${msg.role === 'user' ? 'bg-blue-600 text-white' : 'bg-gray-100 text-gray-900'}`}>
