...
import { Upload, FileSpreadsheet, Download, X, MapPin, PhoneCall, MessageCircle, Camera, MessageSquare, Globe, Plus } from 'lucide-react';
...
export default function CustomersPage() {
  ...
  const [customerForm, setCustomerForm] = useState({ name: '', phone: '', city: '', address: '', company: '', tax_office: '', identity: '', birthday: '', notes: '' });
  ...
  const handleAddCustomer = async () => {
    ...
    await fetch(`/api/customers/${tid}`, {
      method: 'POST', headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        name: customerForm.name, phone: customerForm.phone, city: customerForm.city, address: customerForm.address,
        company_name: customerForm.company, tax_office: customerForm.tax_office, identity_number: customerForm.identity,
        birth_date: customerForm.birthday, notes: customerForm.notes,
      }),
    });
    setShowAddCustomer(false);
    setCustomerForm({ name: '', phone: '', city: '', address: '', company: '', tax_office: '', identity: '', birthday: '', notes: '' });
    load();
  };
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            <button onClick={() => setShowAddCustomer(true)}
              className="inline-flex items-center gap-1 px-3 py-1.5 bg-indigo-500 hover:bg-indigo-600 text-white rounded-lg text-sm font-medium shadow-sm transition-all">
            ...
            <div key={c.id as string} onClick={() => selectCustomer(c)}
              ...
              <div className="flex items-center justify-between gap-2">
                <div className="flex items-center gap-2 min-w-0">
                  ...
                  {src && chInfo[src] && (
                    (() => { const Icon = chInfo[src].Icon; return (
                    <span className={`inline-flex items-center gap-1 text-[10px] px-1.5 py-0.5 rounded-full font-medium shrink-0 ${chInfo[src].cls}`}>
                      <Icon size={11} /> {chInfo[src].label}
                    </span>
                    ); })()
                  )}
                </div>
